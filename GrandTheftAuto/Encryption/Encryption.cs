...
namespace SADL.GrandTheftAuto.Encryption
{
    /// <summary>
    /// Represents the game encryption algorithm
    /// </summary>
    public class Encryption : IEncryption
    {
        ...
        /// </summary>
        /// <param name="data">Data to be decrypted</param>
        /// <param name="key">Key to use in the decryption process</param>
        ...
        public static byte[] DecryptBlock(byte[] data, uint[] key)
        {
            ...
            buffer = DecryptRoundA(buffer, subKeys[0], Constants.PC_NG_DECRYPT_TABLES[0]);
            buffer = DecryptRoundA(buffer, subKeys[1], Constants.PC_NG_DECRYPT_TABLES[1]);
            for (int k = 2; k <= 15; k++)
                buffer = DecryptRoundB(buffer, subKeys[k], Constants.PC_NG_DECRYPT_TABLES[k]);
            buffer = DecryptRoundA(buffer, subKeys[16], Constants.PC_NG_DECRYPT_TABLES[16]);

            return buffer;
        }

        /// <summary>
        /// Decryption based on the A round
        /// </summary>
        /// <param name="data">Data to be decrypted</param>
        /// <param name="key">Key to use in the decryption process</param>
        /// <param name="table">Table wich contains the decryption keys</param>
        ...
        /// <summary>
        /// Decryption based on the B round
        /// </summary>
        /// <param name="data">Data to be decrypted</param>
        /// <param name="key">Key to use in the decryption process</param>
        /// <param name="table">Table wich contains the decryption keys</param>
