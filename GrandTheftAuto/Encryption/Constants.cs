...
using SADL.Rage.Helpers.Hash;
using SADL.GrandTheftAuto.Encryption.Helpers;
using SADL.GrandTheftAuto.Encryption.Exceptions;

namespace SADL.GrandTheftAuto.Encryption
{
    /// <summary>
    /// Contains the known AES keys and NG keys, decryption and lookup tables
    /// </summary>
    ...
    /// <summary>
    /// Constant manipulator
    /// </summary>
    public static class Constants
    {
        public static byte[] PC_AES_KEY;                                                            // defines aes decryption/encryption key

        public static byte[][] PC_NG_KEYS;                                                          // defines ng decryption/encryption expanded keys    

        public static uint[][][] PC_NG_DECRYPT_TABLES;                                              // defines decryption tables

        public static byte[] PC_LUT;                                                                // defines hash lookup-table

        /// <summary>
        /// Generates executable data based on the known constants
        /// </summary>
        public static void Generate(byte[] exeData)
        {
            ...
            PC_AES_KEY = HashSearcher.SearchHash(exeStr, HashConstants.PC_AES_KEY_HASH, length: 0x20);
            if (PC_AES_KEY == null)
            {
                throw new KeyNotFoundException("AES key not found!");
            }
            Console.WriteLine("AES key has been found!");

            // searches for NG keys
            PC_NG_KEYS = HashSearcher.SearchHashes(exeStr, HashConstants.PC_NG_KEY_HASHES, length: 0x110);
            ...
            Console.WriteLine("NG keys have been found!");

            // searches for NG decryption tables
            byte[][] tabs = HashSearcher.SearchHashes(exeStr, HashConstants.PC_NG_DECRYPT_TABLE_HASHES, length: 0x400);
            ...
            Console.WriteLine("NG decrypt tables have been found!");
            ...
            for (int i = 0; i < 17; i++)
            {
                ...
                for (int j = 0; j < 16; j++)
                {
                    byte[] buf = tabs[j + 16 * i];
                    PC_NG_DECRYPT_TABLES[i][j] = new uint[256];
                    Buffer.BlockCopy(buf, 0, PC_NG_DECRYPT_TABLES[i][j], 0, 1024);
                }
            }

            // searches for NG LUTs
            PC_LUT = HashSearcher.SearchHash(exeStr, HashConstants.PC_LUT_HASH, length: 0x100);
            if (PC_LUT == null)
            {
                throw new KeyNotFoundException("LUT not found!");
            }
            Console.WriteLine("NH hash LUTs have been found!");
        }

        /// <summary>
        /// Loads keys from a specified path
        /// </summary>
        /// <param name="path">Path to .dat file</param>
        ...
        /// <summary>
        /// Saves keys to a specified path
        /// </summary>
        /// <param name="path">Path to .dat file</param>
